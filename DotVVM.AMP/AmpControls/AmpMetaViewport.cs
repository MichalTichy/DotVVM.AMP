...
namespace DotVVM.AMP.AmpControls
{
    public class AmpMetaViewport : HtmlGenericControl, IAmpControl
    {
        ...
        {
            var correctWidth = ViewPortProperties.Contains(new KeyValuePair<string, string>("width", "device-width"));
            var correctMinScale = ViewPortProperties.Contains(new KeyValuePair<string, string>("minimum-scale", "1"));

            if (!correctWidth)
            {
                throw new AmpException($"Amp meta viewport requires width:device-width");
            }
            if (!correctMinScale)
            {
                throw new AmpException($"Amp meta viewport requires minimum-scale:1");
            }
        }
    }
}